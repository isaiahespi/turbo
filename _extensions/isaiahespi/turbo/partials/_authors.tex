...
\textsc{$it.name.literal$}$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$
$for(it.affiliations/first)$\\[1ex]
\normalsize $it.name$
$endfor$\\
$if(it.email)$
...
% Below is the newer stuff from the quarto-simple-article _authors.tex file
